...
onEvent("recipes", (event) => {
  ...
  rawOres.forEach((ore) => {
    event.remove({ id: `immersiveengineering:crusher/raw_ore_${ore}` });
    event.remove({ id: `immersiveengineering:crusher/raw_block_${ore}` });

    event.recipes.immersiveengineering.crusher(
      Item.of(`create:crushed_raw_${ore}`, 2),
      getItemIdByOreName(ore),
      [Item.of(`create:crushed_raw_${ore}`).withChance(0.75)]
    );

    event.recipes.immersiveengineering.crusher(
      Item.of(`create:crushed_raw_${ore}`, 18),
